NEWACTION('Settings/read', {
	...
	action: async function($) {
		var items = await DATA.find('op.cl_config').fields('id,value,type').in('id', 'icon,name,url,mail_smtp,mail_smtp_options,language,mail_from,$tms,secret_tms,allow_token,token,newtab'.split(',')).promise($);
		var model = {};
...
NEWACTION('Settings/save', {
	name: 'Save settings',
	input: 'name:String, url:URL, language:Lower, mail_smtp:String, mail_smtp_options:JSON, mail_from:Email, icon:String, $tms:Boolean, secret_tms:String, allow_token:Boolean, token:String, newtab:Boolean',
