NEWACTION('Settings/read', {
	...
	action: async function($) {
		var items = await DATA.find('op.cl_config').fields('id,value,type').in('id', 'icon,name,url,mail_smtp,mail_smtp_options,language,mail_from,allow_tms,secret_tms,allow_token,token'.split(',')).promise($);
		var model = {};
